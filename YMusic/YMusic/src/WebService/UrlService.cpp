...
//测试
static CRequest* MakeRequestWithParams2(emRequestT reType,const char* szParams)
{
	return MakeRequestWithTag(reType,"/YMusic/login.php",szParams,0);
}
//
/************************************************************************/
/*                                                                      */
...
//测试
CRequest* Requester::MakeLogin2(const UTF8STR& sUserName,const UTF8STR& sPasswordMd5)
{//email=389465209@qq.com&password=happy2010&app_name=radio_desktop_win&version=100
	std::string sFunc("/YMusic/login.php");
	std::vector<std::pair<std::string,std::string> > v;

	v.push_back(std::pair<std::string,std::string>("email",sUserName));
	v.push_back(std::pair<std::string,std::string>("password",sPasswordMd5));
	v.push_back(std::pair<std::string,std::string>("app_name","radio_desktop_win"));
	v.push_back(std::pair<std::string,std::string>("version","100"));
	std::string strUrl = GenSigUrl2(sFunc,v).c_str();
	return MakeRequestWithParams2(WSRT_LOGIN,strUrl.c_str());
}
std::string Requester::GenSigUrl2(const std::string& sMethod, const std::vector<std::pair<std::string,std::string> >& v)
{
	std::string sSigUrl;
	std::string sValue;
	std::string sMd5;
