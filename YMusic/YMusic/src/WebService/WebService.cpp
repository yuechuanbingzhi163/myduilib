...
#include "../Utilty/win_io/win_io_base.h"
...
{
#if defined (_DEBUG) || defined (_YTING_BETA)
//	_serversIp.push_back(std::string("dev.xiami.com"));
	_serversIp.push_back(std::string("localhost"));
#else
	_serversIp.push_back(std::string("localhost"));
#endif // _DEBUG

	_serversIp.push_back(std::string("127.0.0.1"));
...
{
	assert(pRequest);
	onAsynRequest(pRequest,pRequest->GetRequestType()==url::WSRT_LOGIN);
}
...
void CWebServiceMgr::SendRequest(lpRequest pRequest,CBuffer& buffer)
{
	...
	do 
	{
		...
		{
			while(theNet->waitReadable(h,10))//存在可读数据
			{
				if(buffer.full())
					buffer.resize();
...
void CWebServiceMgr::onAsynRequest(lpRequest pRequest,bool bPost)
{
	...
	if(pRequest)
	{
		sLog("CWebServiceMgr::onAsynRequest:%d,method:%s,time:%d,istimeout:%d,iscatched:%d",
			pRequest->GetRequestType(),
			(bPost?"doPost":"doGet"),
			clock() - pRequest->GetCreateTime(),
			pRequest->IsTimeOut(),
			pRequest->IsCatched());
...
void CWebServiceMgr::OnRequest(lpRequest pRequest,bool bPost)
{
	assert(pRequest);
	if(!pRequest) return;
	sLog("CWebServiceMgr::OnRequest:%d,method:%s,time:%d,istimeout:%d,iscatched:%d",
		pRequest->GetRequestType(),
		(bPost?"doPost":"doGet"),
		clock() - pRequest->GetCreateTime(),
		pRequest->IsTimeOut(),
...
void CWebServiceMgr::OnResponse(lpRequest pRequest,CBuffer& buffer)
{
	...
	if(bNeedParse)
	{
		sLog("CWebServiceMgr::OnResponse:%d time:%d,istimeout:%d,iscatched:%d",
			pRequest->GetRequestType(),
			clock() - pRequest->GetCreateTime(),
			pRequest->IsTimeOut(),
			pRequest->IsCatched());
...
void CWebServiceMgr::onConnected(hLinker h)
{
	...
	if(pInfo)
	{
		...
		win_io_base win_io;
		win_io.create(_T("header"));
		unsigned int nwrite = 0;

		win_io.write(sbuf.c_str(),sbuf.size(),&nwrite);
