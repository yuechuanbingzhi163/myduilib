...
*****************************************************************************************/
struct md5_hash_value
{
	unsigned int n_value1;
	unsigned int n_value2;
	unsigned int n_value3;
	unsigned int n_value4;
	md5_hash_value();
	md5_hash_value(const unsigned char* p_buf);
	bool is_valid() const
	{
		return n_value1 != 0 || n_value2 != 0 || n_value3 != 0 || n_value4 != 0;
	}
	std::string to_string() const;
	static std::string int32_2_str(unsigned int n_value);
	bool operator< (const md5_hash_value& rhs) const
	{
		return less_than(*this, rhs);
	}
	static bool less_than(const md5_hash_value& l, const md5_hash_value& r)
	{
		if (l.n_value1 < r.n_value1)
			return true;
		else if (l.n_value1 == r.n_value1)
		{
			if (l.n_value2 < r.n_value2)
				return true;
			else if (l.n_value2 == r.n_value2)
			{
				if (l.n_value3 < r.n_value3)
					return true;
				else if (l.n_value3 == r.n_value3)
				{
					if (l.n_value4 < r.n_value4)
						return true;
					else 
						return false;
				}
...
struct md5_hash_less: public std::binary_function<const md5_hash_value&,
	const md5_hash_value, bool> 
{
	bool operator()(const md5_hash_value& l, const md5_hash_value r) const
	{
		return md5_hash_value::less_than(l, r);
	}
};
...
{
public:
	static const unsigned int MD5_BUF_SIZE = 16;				//unsigned char buf size
	static const unsigned int MD5_HASH_SIZE = 32;			//32个字节明文
	//interface functions for the RSA MD5 calculation
	static bool get_md5(const unsigned char* p_buf, unsigned int n_len, unsigned char* p_md5);
	static md5_hash_value get_hash_value(const unsigned char* p_buf, unsigned int n_len);
	static std::string get_md5(const unsigned char* p_buf, unsigned int n_length);
	static std::string get_md5(const TCHAR* p_file_path);
	//对文件的部分做哈希
	static std::string get_md5(const TCHAR* p_file_path, unsigned int n_begin, unsigned int n_len);
	static bool same_md5(const unsigned char* p_src, const unsigned char* p_dest);
	bool add_data(const unsigned char* p_buf, unsigned int n_length);
	std::string finish();
	void clear();
public:
	//constructor/destructor
	md5_checksum();
	virtual ~md5_checksum() {};
protected:
	//RSA MD5 implementation
	void Transform(const unsigned char Block[64]);
	void Update(const unsigned char* Input, unsigned int nInputLen);
	std::string Final();
	void Final_core(unsigned char* p_buf, unsigned int n_len);
	...
	//utility functions
	inline void uint_2_uchar(unsigned char* p_output, unsigned int* p_input, unsigned int n_length);
	inline void uchar_2_uint(unsigned int* p_output, const unsigned char* p_input, unsigned int n_length);
private:
	unsigned char ms_buffer[64];		//input buffer
	unsigned int mn_count[2];			//number of bits, modulo 2^64 (lsb first)
	unsigned int mn_MD5[4];			//MD5 checksum
};
