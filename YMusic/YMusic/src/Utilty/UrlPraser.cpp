...
{
	UrlPraser::UrlPraser(const char* url)
	{
		Init(url);
	}

	bool UrlPraser::Init(const char* url)
	{
		const char *tmpstr;
		const char *curstr;
		int len;
	...
	std::string UrlPraser::GetParameter(const std::string query,const char* param)
	{
		assert(param);
		if(!param) return "";
	...
	std::string UrlPraser::GetParameter(const char* param) 
	{
		assert(param);
		if(!param) return "";
