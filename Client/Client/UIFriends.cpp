...
Node* CFriendsUI::AddNode(const FriendListItemInfo& item, Node* parent)
{
	...
	{
#if defined(UNDER_WINCE)
		_stprintf(szBuf, _T("%s"), item.nick_name.GetData());
#else
	...
	if (!item.folder && !item.description.IsEmpty())
	{
		...
		{
#if defined(UNDER_WINCE)
			_stprintf(szBuf, _T("<x 20><c #808080>%s</c>"), item.description.GetData());
#else
...
Node* CFriendsUI::AddNode(const User& user, Node* parent )
{
	FriendListItemInfo item;
	CDuiString sIpAndPort = user.m_userIp;
	sIpAndPort = sIpAndPort  + _T("_");
	sIpAndPort += user.m_userPort;
	item.id = sIpAndPort;
	item.nick_name = user.m_loadName;
	item.folder = false;
//	item.logo = _T("list/default.png");
	if(user.m_loadName==theUser.m_loadName)
		item.description = _T("myself");
	else item.description = _T("friend");

	return AddNode(item,parent);
}
// node->data().text_ = item.nick_name;
// node->data().value = item.id;
User CFriendsUI::GetUser(Node* node)
{
	User user;
	if(node==NULL)
		return user;
	user.m_loadName = node->data().text_;
	CString sIpAndPort = node->data().value;
	int iPos = sIpAndPort.Find(_T("_"));
	if(iPos>-1)
	{
		CString sIp = sIpAndPort.Left(iPos-1);
		CString sPort = sIpAndPort.Right(sIpAndPort.GetLength()-iPos);
		user.m_userIp = sIp;
		user.m_userPort = sPort;
	}
	return user;
}
...
void CFriendsUI::SetUserCount(Node* node,int iCount)
{
...
#if defined(UNDER_WINCE)
		_stprintf(szCount, _T("[共%d人]"), iCount);
#else
		_stprintf_s(szCount, MAX_PATH - 1, _T("[共%d人]"), iCount);
#endif
