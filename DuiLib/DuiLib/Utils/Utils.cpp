...
namespace DuiLib
{
	...
	bool CDuiString::IsSame(LPCTSTR pstr) const
	{
		return 0 == this->Compare(pstr);
	}

	bool CDuiString::IsSameNoCase(LPCTSTR pstr) const
	{
		return 0 == this->CompareNoCase(pstr);
	}
...
#ifdef _UNICODE
		int iRet = vswprintf(szBuffer,lengthof(szBuffer) - 2, sFormat, argList);
#else
		int iRet = vsprintf_s(szBuffer,lengthof(szBuffer) - 2,sFormat,argList);
#endif
	...
	CDuiString& CDuiString::FormatString(LPCTSTR pstrFormat, ...)
	{
	...
	CDuiString& CDuiString::SmallFormatString(LPCTSTR pstrFormat,...)
	{
		static CDuiString sReturn ;
		CDuiString sFormat = pstrFormat;
		TCHAR szBuffer[64] = { 0 };
...
#	endif
	...
	std::string CDuiString::str() const
	{
		std::string s_result("");
		int dwMinSize;
		dwMinSize = WideCharToMultiByte(::GetACP(),NULL,(LPCWSTR)m_pstr,-1,NULL,0,NULL,FALSE); //计算长度
