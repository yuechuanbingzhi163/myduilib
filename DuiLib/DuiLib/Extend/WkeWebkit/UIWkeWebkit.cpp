...
namespace DuiLib{
	...
	LPCTSTR CWkeWebkitUI::GetClassName()
	{
	...
	void CWkeWebkitUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		{
			SetURL(wkeString(pstrValue));
		}
		CControlUI::SetAttribute(pstrName,pstrValue);
	}

	void CWkeWebkitUI::SetURL( wkeString strValue)
	{
		m_pWindow->m_strUrl = strValue;
		m_pWindow->m_pWebView->loadURL(strValue.c_str());
	}

	void CWkeWebkitUI::SetFile( wkeString strValue)
	{
		m_pWindow->m_strUrl = strValue;
		m_pWindow->m_pWebView->loadFile(strValue.c_str());
	}

	wkeString CWkeWebkitUI::RunJS(wkeString strValue)
	{
		wkeString retS;
		jsValue jsRet = m_pWindow->m_pWebView->runJS(strValue.c_str());
		#ifdef _UNICODE
			retS.append(jsToStringW(m_pWindow->m_pWebView->globalExec(), jsRet));
			return retS.c_str();
		#else
			retS.append(jsToString(m_pWindow->m_pWebView->globalExec(), jsRet));
			return retS.c_str();
		#endif	
	}
