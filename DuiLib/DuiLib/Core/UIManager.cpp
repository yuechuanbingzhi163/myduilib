...
namespace DuiLib {
	...
	public:
		...
		BOOL SetActivate(BOOL bActivate);

		HWND TooltipWnd() const;
	protected:
		HWND    _hWndTooltip;
	};

	CTooptip::CTooptip():_hWndTooltip(NULL)
	{
	}
	...
	BOOL CTooptip::Setup()
	{
		...
		return _hWndTooltip != NULL;
	}

	VOID CTooptip::Uninstall()
	{
		if(_hWndTooltip != NULL && ::IsWindow(_hWndTooltip))
		{
	...
	BOOL CTooptip::AddTool(HWND hWndTool)
	{
		if(_hWndTooltip == NULL) return FALSE;
		...
		::SendMessage(_hWndTooltip, TTM_ACTIVATE, TRUE, 0);

		return TRUE;
	}

	BOOL CTooptip::RemoveTool(HWND hWndTool)
	{
		if(_hWndTooltip == NULL) return FALSE;
	...
	BOOL CTooptip::SetActivate(BOOL bActivate)
	{
		if(_hWndTooltip == NULL) return FALSE;

		::SendMessage(_hWndTooltip, TTM_ACTIVATE, bActivate, 0);

		return TRUE;
	}

	HWND CTooptip::TooltipWnd() const
	{
		return _hWndTooltip;
	}
/////////////////////////////////////////////////////////////////////////////////////
...
//m_hbmpBackground(NULL),// modify by dfn.li 2013-8-31
m_pBmpAlphaCanvasBits(NULL),
m_pBmpRgbCanvasBits(NULL),
m_hBmpAlphaCanvas(NULL),
m_hBmpRgbCanvas(NULL), //modify end
m_hwndTooltip(NULL),
...
m_pParentResourcePM(NULL)
{
	...
	::ZeroMemory(&m_rcCaption_Bottom,sizeof(m_rcCaption_Bottom));//modify by dfn.li 2013-7-6 9:30
...
//    if( m_hbmpBackground != NULL ) ::DeleteObject(m_hbmpBackground); //modify by dfn.li 2013-8-31
	if( m_hBmpAlphaCanvas != NULL ) ::DeleteObject(m_hBmpAlphaCanvas);
	if( m_hBmpRgbCanvas != NULL ) ::DeleteObject(m_hBmpRgbCanvas); //modify end
...
RECT& CPaintManagerUI::GetBottomCaptionRect()
{
	return m_rcCaption_Bottom;
}

void CPaintManagerUI::SetBottomCaptionRect(RECT& rcCaption)
{
	m_rcCaption_Bottom = rcCaption;
}
				...
				if( m_bUpdateNeeded )
				{
					...
					if( !::IsRectEmpty(&rcClient) )
					{
						if( m_pRoot->IsUpdateNeeded() )
						{
							...
							if( m_hDcBackground != NULL ) ::DeleteDC(m_hDcBackground);
							if( m_hBmpAlphaCanvas != NULL ) ::DeleteObject(m_hBmpAlphaCanvas);
							if( m_hBmpRgbCanvas != NULL ) ::DeleteObject(m_hBmpRgbCanvas);
//							if( m_hbmpOffscreen != NULL ) ::DeleteObject(m_hbmpOffscreen);
							m_hDcBackground = NULL;
							m_hBmpAlphaCanvas = NULL;
							m_hBmpRgbCanvas = NULL;
							m_pBmpAlphaCanvasBits = NULL;
							m_pBmpRgbCanvasBits = NULL;
//							m_hbmpOffscreen = NULL;
						}
				...
				if(m_bOffscreenPaint && m_hBmpAlphaCanvas == NULL)
				{
					m_hDcBackground = ::CreateCompatibleDC(m_hDcPaint);
					BITMAPINFO bmi;
					...
					m_hBmpAlphaCanvas = ::CreateDIBSection(m_hDcPaint, &bmi, DIB_RGB_COLORS, 
						(void**)&m_pBmpAlphaCanvasBits, NULL, 0);

					BITMAPINFO bmi2;
					::ZeroMemory(&bmi2, sizeof(BITMAPINFO));
					...
					bmi2.bmiHeader.biSizeImage = width * height * 4;
					bmi2.bmiHeader.biClrUsed = 0;   
					m_hBmpRgbCanvas = ::CreateDIBSection(m_hDcPaint, &bmi2, DIB_RGB_COLORS, 
						(void**)&m_pBmpRgbCanvasBits, NULL, 0);

					ASSERT(m_hDcBackground);
					ASSERT(m_hBmpAlphaCanvas);
					ASSERT(m_hBmpRgbCanvas);
				}
				...
				for(int i = mirrorTop; i < mirrorBottom; ++i)
				{
					...
					{
						int x = (i * width + j) * 4;
						m_pBmpAlphaCanvasBits[x + 0] = 0;
						m_pBmpAlphaCanvasBits[x + 1] = 0;
						m_pBmpAlphaCanvasBits[x + 2] = 0;
						m_pBmpAlphaCanvasBits[x + 3] = 0;

						m_pBmpRgbCanvasBits[x + 0] = 0;
						m_pBmpRgbCanvasBits[x + 1] = 0;
						m_pBmpRgbCanvasBits[x + 2] = 0;
						m_pBmpRgbCanvasBits[x + 3] = 0;
					}
				}

				//在m_hbmpAlphaCanvas画布上绘制透明层
				::SelectObject(m_hDcBackground, m_hBmpAlphaCanvas);

				CRenderClip clip;
				CRenderClip::GenerateClip(m_hDcBackground, rcPaint, clip);
				m_pRoot->CControlUI::DoPaint(m_hDcBackground, rcPaint);

				//在m_hbmpRgbCanvas画布上绘制RGB颜色值
				::SelectObject(m_hDcBackground, m_hBmpRgbCanvas);
				m_pRoot->DoPaint(m_hDcBackground, rcPaint);

				::SelectObject(m_hDcBackground, m_hBmpAlphaCanvas);

				//合并两张画布，m_hbmpAlphaCanvas只提供透明层，m_hbmpRgbCanvas只提供RGB颜色值
				for(int i = mirrorTop; i < mirrorBottom; ++i)
				{
					for(int j = morrorLeft; j < morrorRight; ++j)
					{
						int x = (i * width + j) * 4;
						m_pBmpAlphaCanvasBits[x + 0] = m_pBmpRgbCanvasBits[x + 0];
						m_pBmpAlphaCanvasBits[x + 1] = m_pBmpRgbCanvasBits[x + 1];
						m_pBmpAlphaCanvasBits[x + 2] = m_pBmpRgbCanvasBits[x + 2];
					}
				}
...
void CPaintManagerUI::AddCustomStyle(LPCTSTR pStrStyleName,CStdStringPtrMap* pStyleMap)
{
	if (pStyleMap != NULL)
	{
		if (m_aUserCustomStyle.Find(pStrStyleName) == NULL)
			m_aUserCustomStyle.Set(pStrStyleName,(LPVOID)pStyleMap);
	}
}
LPCTSTR CPaintManagerUI::GetCustomStyle(LPCTSTR pStrStyleName,LPCTSTR pStrAttrName) const
{
	if(pStrStyleName == NULL) return NULL;
	if(pStrAttrName==NULL) return NULL;
	CStdStringPtrMap* pPtrMap = static_cast<CStdStringPtrMap*>(m_aUserCustomStyle.Find(pStrStyleName));
	if(pPtrMap == NULL) return NULL;
	CDuiString* pStrAttrValue = static_cast<CDuiString*>(pPtrMap->Find(pStrAttrName));
	 
	if( pStrAttrValue ) return pStrAttrValue->GetData();
	else return NULL;
}
CStdStringPtrMap* CPaintManagerUI::GetCustomStyle(LPCTSTR pStrStyleName)
{
	if(pStrStyleName == NULL) return NULL;
	CStdStringPtrMap* pPtrMap = static_cast<CStdStringPtrMap*>(m_aUserCustomStyle.Find(pStrStyleName));
	return pPtrMap;
...
{
	CDuiString* pStyleAttr;
	for( int i = 0; i< m_aUserCustomStyle.GetSize(); i++ ) {
		if(LPCTSTR key = m_aUserCustomStyle.GetAt(i)) {
			CStdStringPtrMap* pPtrMap = static_cast<CStdStringPtrMap*>(m_aUserCustomStyle.Find(key));
			if(pPtrMap != NULL)
			{
				for (int j=0;j<pPtrMap->GetSize();j++)
				{
					if (LPCTSTR attrkey = pPtrMap->GetAt(j))
					{
						pStyleAttr = static_cast<CDuiString*>(pPtrMap->Find(attrkey));
						delete pStyleAttr;
					}
				}
				pPtrMap->RemoveAll();
...
void CPaintManagerUI::PlaySound(LPCTSTR pStrName)
{
	ASSERT(pStrName);
	if(!pStrName) return ;
	CDuiString sFile = CPaintManagerUI::GetResourcePath() + pStrName;
	wchar_t* pFileName = (wchar_t*)(LPTSTR)sFile.GetData();
	int nRet = _waccess(pFileName,0);
	if(-1 != nRet ) ::PlaySound(sFile.GetData(),NULL,SND_ASYNC);
}
