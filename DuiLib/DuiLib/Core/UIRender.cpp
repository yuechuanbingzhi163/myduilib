                                          ...
                                          LPCTSTR pStrImage, LPCTSTR pStrModify)
{
	...
	bool CRenderEngine::DrawImageString(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, 
	LPCTSTR pStrImage, LPCTSTR pStrModify, bool bNeedAlpha, BYTE bNewFade,int hIndex,int vIndex)
{
	if ((pManager == NULL) || (hDC == NULL)) return false;
		...
		while( *pStrImage != _T('\0') ) {
			...
			if( !sValue.IsEmpty() ) {
				if( sItem == _T("file") || sItem == _T("res") ) {
					...
					if(hIndex > 0)
					{
						const int w = rcBmpPart.right - rcBmpPart.left;
						rcBmpPart.left  += w*hIndex;
						rcBmpPart.right += w*hIndex;
					}
					else if(vIndex > 0)
					{
						const int h = rcBmpPart.bottom - rcBmpPart.top;
						rcBmpPart.top  += h*vIndex;
						rcBmpPart.bottom += h*vIndex;
					}
				}
				else if( sItem == _T("corner") ) {
					rcCorner.left = _tcstol(sValue.GetData(), &pstr, 10);  ASSERT(pstr);    
