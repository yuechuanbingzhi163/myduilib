...
bool CControlUI::IsClass(const LPCTSTR pstrClass)
{
	return ((GetClass())!=NULL)&&(_tcscmp(pstrClass,GetClass()) == 0);
}
bool CControlUI::IsName(const LPCTSTR pstrName)
{
	return (GetName().GetData()!=NULL)&&(_tcscmp(pstrName,GetName()) == 0);
}
...
LPCTSTR CControlUI::GetClassName()
{
	return _T("ControlUI");
}
...
// }
bool CControlUI::DrawImage(HDC hDC, LPCTSTR pStrImage, LPCTSTR pStrModify, bool bNeedAlpha, BYTE bNewFade,int hIndex,int vIndex)
{
	return CRenderEngine::DrawImageString(hDC, m_pManager, m_rcItem, m_rcPaint, pStrImage, pStrModify, bNeedAlpha, bNewFade,hIndex,vIndex);
}
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("style")) == 0 ) SetStyleName(pstrValue); //add by dfn.li
	else if( _tcscmp(pstrName, _T("soundnormal")) == 0 ) SetSoundNameOver(pstrValue);
	else if( _tcscmp(pstrName, _T("soundpushed")) == 0 ) SetSoundNameDown(pstrValue);
}
void CControlUI::SetCustomStyle(LPCTSTR lpstrStyleName)
{
	ASSERT(lpstrStyleName);
	if(!m_pManager) return ;
	CStdStringPtrMap* pPtrMap = m_pManager->GetCustomStyle(lpstrStyleName);
	if (pPtrMap)
	{
		for (int i=0;i<pPtrMap->GetSize();++i)
		{
			CDuiString* pStrAttrValue = NULL;
			if (LPCTSTR pStrAttrKey = pPtrMap->GetAt(i))
			{
				pStrAttrValue = static_cast<CDuiString*>(pPtrMap->Find(pStrAttrKey));
				if(pStrAttrValue)SetAttribute(pStrAttrKey,pStrAttrValue->GetData());
			}
		}
	}
}
void CControlUI::SetCustomStyle2(CStdStringPtrMap* pPtrMap)
{
	if (pPtrMap)
	{
		for (int i=0;i<pPtrMap->GetSize();++i)
...
// Modify by dfn.li
//************************************
// 函数名称: SetSoundNameDown
// 返回类型: void
// 参数信息: LPCTSTR pstrText
// 函数说明: 
//************************************
void CControlUI::SetSoundNameDown(LPCTSTR pstrText)
{
	m_sSoundNameDown = pstrText;
	Invalidate();
}

void CControlUI::SetSoundNameOver(LPCTSTR pstrText)
{
	m_sSoundNameOver = pstrText;
	Invalidate();
}

CDuiString CControlUI::GetSoundNameDown() const
{
	return m_sSoundNameDown;
}

CDuiString CControlUI::GetSoundNameOver() const
{
	return m_sSoundNameOver;
}

CDuiString CControlUI::GetStyleName() const
{
	return m_sStyleName;
}
void CControlUI::SetStyleName(LPCTSTR pstrValue)
{
	if( !pstrValue  || _tclen(pstrValue) <= 0 || !m_pManager )
		return ;

	CStdStringPtrMap* pPtrMap = m_pManager->GetCustomStyle(pstrValue);
	if( !pPtrMap )
		return ;
