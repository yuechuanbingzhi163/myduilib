...
{
    m_pCallback = pCallback;
    CMarkupNode root = m_xml.GetRoot(); //tag = Window
            ...
            if( _tcscmp(pstrClass, _T("Image")) == 0 ) {
				...
				for( int i = 0; i < nAttributes; i++ ) {
					...
					if( _tcscmp(pstrName, _T("name")) == 0 ) {
						...
						if (pStyleAttr != NULL&&pStyleMap)
						{
							if (pStyleMap->Find(pstrName) == NULL)
								pStyleMap->Set(pstrName, (LPVOID)pStyleAttr);
							else
								delete pStyleAttr;
						}
					}
				...
				if( pStyleName ) {
					pManager->AddCustomStyle(pStyleName,pStyleMap);
				}
			}
        ...
        if( _tcscmp(pstrClass, _T("Window")) == 0 ) {
            if( pManager->GetPaintWindow() ) {
                ...
                for( int i = 0; i < nAttributes; i++ ) {
					...
					//modify by dfn.li 2013-7-6 9:35
					else if ( _tcscmp(pstrName, _T("bottom_caption")) == 0 ){
						RECT rcBottomCaption = { 0 };
						LPTSTR pstr = NULL;
						rcBottomCaption.left = _tcstol(pstrValue, &pstr, 10);  ASSERT(pstr);    
						rcBottomCaption.top = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr);    
						rcBottomCaption.right = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);    
						rcBottomCaption.bottom = _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr);    
						pManager->SetBottomCaptionRect(rcBottomCaption);
					}
...
CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
			...
			CDuiString szNodeName(pstrClass);
			szNodeName = CDuiString(_T("C")) + szNodeName;
			szNodeName +=  _T("UI");
            ...
            case 6:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_HOTKEY) == 0 )            pControl = new CHotKeyUI;
                break;
            case 7:
            ...
            case 9:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_SEPBUTTON) == 0 )         pControl = new CSeqButtonUI;
				else if( _tcscmp(pstrClass, DUI_CTR_WKEWEBKIT) == 0 )		  pControl = new CWkeWebkitUI;
                break;
            case 10:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_FADEOPTION) == 0 )        pControl = new CFadeOptionUI;
                break;
			case 11:
				if (_tcscmp(pstrClass, DUI_CTR_CHILDLAYOUT) == 0)			  pControl = new CChildLayoutUI;
				else if( _tcscmp(pstrClass, DUI_CTR_ANIMCONTROL) == 0 )       pControl = new CAnimControlUI;
				else if( _tcscmp(pstrClass, DUI_CTR_ALBUMBUTTON) == 0 )       pControl = new CAlbumButtonUI;
				break;
			case 12:
				if(_tcscmp(pstrClass, DUI_CTR_FADECHECKBOX) == 0)             pControl = new CFadeCheckBoxUI;
				break;
            case 14:
