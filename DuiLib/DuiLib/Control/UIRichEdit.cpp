...
//  member functions
//////////////////////////////////////////////////////////////////////
void CImageDataObject::InsertBitmap(LPCTSTR lpValue)
{
	HINSTANCE hInst = ::GetModuleHandle(NULL);
	HBITMAP hBitmap = LoadBitmap(hInst,lpValue);
	InsertBitmap(hBitmap);
}
void CImageDataObject::InsertBitmap(HBITMAP hBitmap)
{
	if(m_pRichEdit == NULL) return ;
	IRichEditOle* pRichEditOle = m_pRichEdit->GetRichEditOle();
	if(pRichEditOle == NULL ) return ;

	SCODE sc;

	// Get the image data object
//	CImageDataObject *pods = this;
//	LPDATAOBJECT lpDataObject = NULL;
	this->QueryInterface(IID_IDataObject, (void **)&m_lpDataObject);

	this->SetBitmap(hBitmap);

	// Get the RichEdit container site
//	IOleClientSite *pOleClientSite = NULL;	
	pRichEditOle->GetClientSite(&m_pOleClientSite);
...
//	IOleObject *pOleObject = NULL; 
	m_pOleObject = this->GetOleObject(m_pOleClientSite, m_pStorage);
	...
	CLSID clsid;
	sc = m_pOleObject->GetUserClassID(&clsid);
	if (sc != S_OK)
	{
		//AfxThrowOleException(sc);
	}
	...
	reobject.poleobj = m_pOleObject;
	reobject.polesite = m_pOleClientSite;
...
BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
{
	...
	hModuleRiched20 = LoadLibrary(_T("Riched20.dll"));

	if (NULL == hModuleRiched20)
		goto err;
	else
	{
		ctsFunc = (CTSFunc)GetProcAddress(hModuleRiched20, "CreateTextServices");
...
CRichEditUI::CRichEditUI() :m_pCallback(NULL), m_pTwh(NULL),m_pRichEditOle(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
    ...
    m_iLimitText(cInitTextMax), m_lTwhStyle(ES_MULTILINE),m_bInited(false) ,m_pImageObject(NULL)
{
...
{
    return CRichEditUI::GetClassName();
}

LPCTSTR CRichEditUI::GetClassName()
{
...
bool CRichEditUI::SetImageObject(CImageDataObjectCallback *pImageObject)
{
	if(m_pImageObject)
	{
		m_pImageObject->Release();
...
CImageDataObjectCallback* CRichEditUI::GetImageObject()
{
	return m_pImageObject;
}

CTxtWinHost* CRichEditUI::GetTxWinHost()
{
	return m_pTwh;
}
...
{
	if(m_pImageObject)
		m_pImageObject->InsertBitmap(hBitmap);
}

void CRichEditUI::InsertBitmap(LPCTSTR lpValue)
{
	if(m_pImageObject)
		m_pImageObject->InsertBitmap(lpValue);
}
...
void CRichEditUI::DoInit()
{
	...
	CImageDataObject* pObject = new CImageDataObject(this);
	SetImageObject(pObject);
