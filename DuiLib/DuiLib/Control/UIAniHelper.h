#ifndef _UI_ANIHELPER__H__
#define	_UI_ANIHELPER__H__
#pragma once
...
#pragma pack() // turn byte alignment off
	...
	public:
		CUIAnim(IAnimationCallback* pCallback);
		~CUIAnim();
	...
	class UILIB_API CUIAnimHelper : public IAnimationCallback
	{
	public:
		~CUIAnimHelper()
		{
			...
			while (m_mAniHash.GetSize() > 0)
			{
				if(LPCTSTR key = m_mAniHash.GetAt(0)) {
					data = static_cast<CUIAnim*>(m_mAniHash.Find(key));
					if (data)
					{
						delete data;
						data = NULL;
