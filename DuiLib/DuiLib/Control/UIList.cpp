...
void CListUI::DoEvent(TEventUI& event)
{
    ...
    case UIEVENT_KEYDOWN:
		...
		case _T('A'):
			{
				if(IsMultiSelected() && (event.wKeyState&MK_CONTROL))
				{
...
bool CListUI::SelectItem(int iIndex, bool bTakeFocus,bool bSelected)
{
	if(bSelected)
	{
		...
		const bool isControl = ((GetKeyState(VK_CONTROL) & 0x8000) > 0);
		...
		if( m_iCurSel >= 0 ) {
			...
			if(IsMultiSelected())
			{
				if(isShift)
				{
					const int nMin = (iIndex >= m_iCurSel ? m_iCurSel : iIndex);
					const int nMax = (iIndex < m_iCurSel ?  m_iCurSel : iIndex);
					for (int i=0;i<GetCount();++i)
					{
						CControlUI* pControl = GetItemAt(i);
						if(pControl)
						{
							...
							if( pListItem)
							{
								bool isInRange = (i >= nMin && i <= nMax);
								if(isInRange)
									m_iCurSel = i;
								pListItem->Select(isInRange);
							}
						}
					}
...
void CListUI::SetMultiSelect(bool b)
{
	m_ListInfo.bMultiSelect = b;
}
bool CListUI::IsMultiSelected() const
{
...
LPCTSTR CListHeaderUI::GetClassName()
{
	 return _T("ListHeaderUI");
}
LPCTSTR CListHeaderUI::GetClass() const
{
    return CListHeaderUI::GetClassName();
}
...
LPCTSTR CListHeaderItemUI::GetClassName()
{
	 return _T("ListHeaderItemUI");
}
LPCTSTR CListHeaderItemUI::GetClass() const
{
   return CListHeaderItemUI::GetClassName();
}
...
LPCTSTR CListElementUI::GetClassName()
{
	  return _T("ListElementUI");
}
LPCTSTR CListElementUI::GetClass() const
{
  return CListElementUI::GetClassName();
}
...
bool CListElementUI::Select(bool bSelect)
{
    ...
    m_bSelected = bSelect;
    if( !bSelect && m_pOwner != NULL ) m_pOwner->SelectItem(m_iIndex,false,false);
    Invalidate();
...
LPCTSTR CListLabelElementUI::GetClassName()
{
	 return _T("ListLabelElementUI");
}
LPCTSTR CListLabelElementUI::GetClass() const
{
   return CListLabelElementUI::GetClassName();
}
...
void CListLabelElementUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_RBUTTONDOWN )
    {
        if( IsEnabled() ) {
			...
			{
				const bool isControl = ((GetKeyState(VK_CONTROL) & 0x8000) > 0);
				if(m_pOwner && m_pOwner->IsMultiSelected() && isControl)
					Select(false);
...
LPCTSTR CListTextElementUI::GetClassName()
{
	 return _T("ListTextElementUI");
}
LPCTSTR CListTextElementUI::GetClass() const
{
   return CListTextElementUI::GetClassName();
}
...
/////////////////////////////////////////////////////////////////////////////////////
// CMenuContainerElementUI::CMenuContainerElementUI()
// {
// 
// }
// 
...
// 
// }
// LPCTSTR CListContainerElementUI::GetClassName()
// {
...
{
   return CListContainerElementUI::GetClassName();
}
...
void CListContainerElementUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN)
    {
        if( IsEnabled() ){
			...
			m_dragStart = true;
			m_pManager->SendNotify(this, _T("dragStart"));
        }
        return;
    }
...
// 					Select();
    ...
    {
		m_dragStart = false;
		m_pManager->SendNotify(this, _T("dragEnd"));
        return;
    }
    if( event.Type == UIEVENT_MOUSEMOVE )
    {
		if (m_dragStart)
			m_pManager->SendNotify(this, _T("dragMove"));
        return;
    }
    if( event.Type == UIEVENT_MOUSEENTER )
    {
	...
	if( event.Type == UIEVENT_CONTEXTMENU ) // add by rz.li
	{
