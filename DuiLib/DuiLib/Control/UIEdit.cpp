...
/*	void CEditWnd::Init(CEditUI* pOwner)
	...
	*/
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		{
			//È¥µô°´RETURN¼üÊ± ßË ÉùÏì by rz.li 2013/01/10
		}
		else bHandled = FALSE;
	...
	LPCTSTR CEditUI::GetClassName()
	{
		return _T("EditUI");
	}
	LPCTSTR CEditUI::GetClass() const
	{
		return CEditUI::GetClassName();
	}
	...
	void CEditUI::SetPrompt(LPCTSTR lpszPrompt)
	{
		m_sPrompt = lpszPrompt;
		Invalidate();
	}

	LPCTSTR CEditUI::GetPrompt() const
	{
		return m_sPrompt;
	}
	...
	void CEditUI::SetVisible(bool bVisible)
	{
		...
		if( !IsVisible() && m_pWindow != NULL && IsFocused()) m_pManager->SetFocus(NULL);
	}
