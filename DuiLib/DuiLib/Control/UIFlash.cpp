...
namespace DuiLib
{
	...
	void CFlashUI::DoInit()
	{
		...
		if (m_pFlash&&m_szFlashUrl.IsEmpty() == false)
		{
			m_pFlash->put_WMode(_bstr_t(_T("Transparent") ));	// FlashUI没有实现特定接口，需要完善才能支持
			m_pFlash->put_Movie( _bstr_t(CPaintManagerUI::GetInstancePath() + m_szFlashUrl.GetData()) );
			...
			m_pFlash->CallFunction(request, &response);
