...
namespace DuiLib
{
	...
	//////////////////////////////////////////////////////////////////////
	inline int CUIAnim::TGIFControlExt::GetPackedValue(enum ControlExtValues Value)
	{
	...
	inline int CUIAnim::TGIFLSDescriptor::GetPackedValue(enum LSDPackedValues Value)
	{
		int nRet = (int) m_cPacked;
	...
	inline int CUIAnim::TGIFImageDescriptor::GetPackedValue(enum IDPackedValues Value)
	{
		int nRet = (int) m_cPacked;
	...
	// ctor && dtor
	//////////////////////////////////////////////////////////////////////
	CUIAnim::CUIAnim(IAnimationCallback* pCallback) : m_pCallback(pCallback)
	{
	...
	const TImageInfo* CUIAnim::LoadGIF(LPCTSTR bitmap, LPCTSTR type/* = NULL*/, DWORD mask/* = 0*/)
	{
		LPBYTE pData = NULL;
		DWORD dwSize = 0;
	...
	const TImageInfo* CUIAnim::LoadGIF(const LPBYTE pData, DWORD dwSize, DWORD mask/* = 0*/)
	{
		if (!pData || dwSize==0)
			return NULL;
	...
	bool CUIAnim::IsPlaying() const
	{
		return !m_bExitThread;
	}

	SIZE CUIAnim::GetSize() const
	{
		return m_PictureSize;
	}

	int CUIAnim::GetFrameCount() const
	{
		if (m_pvFrames)
	...
	COLORREF CUIAnim::GetBkColor() const
	{
		return m_clrBackground;
	}

	const TImageInfo* CUIAnim::GetCurImage()
	{
	...
	// protected methods
	//////////////////////////////////////////////////////////////////////
	int CUIAnim::GetNextBlockLen() const
	{
	...
	BOOL CUIAnim::SkipNextGraphicBlock()
	{
		if(!m_pRawData)
			return FALSE;
	...
	UINT CUIAnim::GetSubBlocksLen(UINT nStartingOffset) const
	{
		UINT nRet = 0;
	...
	LPBYTE CUIAnim::GetNextGraphicBlock(UINT* pBlockLen, UINT* pDelay, SIZE* pBlockSize, SIZE* pBlockOffset, UINT* pDisposal)
	{
		if(!m_pRawData)
			return NULL;
...
#ifdef GIF_TRACING
	void CUIAnim::WriteDataOnDisk(CString szFileName, HGLOBAL hData, DWORD dwSize)
	{
		CFile file;
	...
	DWORD __stdcall CUIAnim::_ThreadAnimation(LPVOID pParam)
	{
		assert(pParam);
		CUIAnim* pAni = static_cast<CUIAnim*>(pParam);
		if (!pAni)
			return 1;
	...
	bool CUIAnimHelper::DrawAniImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const CDuiString& sImageName, \
	...
	bool CUIAnimHelper::DrawImageEx(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, 
	...
	const TImageInfo* CUIAnimHelper::GetAniImage(LPCTSTR bitmap)
	{
		CUIAnim* data = static_cast<CUIAnim*>(m_mAniHash.Find(bitmap));
		if (data)
	...
	const TImageInfo* CUIAnimHelper::AddAniImage(HDC hDC, LPCTSTR bitmap, LPCTSTR type/* = NULL*/, DWORD mask/* = 0*/)
	{
		CUIAnim* pAniImage = new CUIAnim(this);
