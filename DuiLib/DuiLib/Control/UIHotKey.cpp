...
namespace DuiLib
{
	...
	LPCTSTR CHotKeyUI::GetClassName()
	{
		return _T("HotKeyUI");
	}
	...
	{
		DWORD hotkey = 0;
		hotkey = MAKEWORD(m_wVirtualKeyCode, m_wModifiers);
		return hotkey;
	}

	void CHotKeyUI::SetHotKey( DWORD hotkey )
	{
		WORD virtualKeyCode = LOBYTE(LOWORD(hotkey));
		WORD modifiers = HIBYTE(LOWORD(hotkey));

		SetHotKey(virtualKeyCode, modifiers);
	}
