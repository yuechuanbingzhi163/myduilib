...
void CActiveXUI::Navigate(BSTR path)
{
	if (m_pFlash)
	{
		m_pFlash->put_Movie(path);
		m_pFlash->Play();
	}
}
...
void CActiveXUI::DoAxActive(IUnknown* pUnknown)
{
	pUnknown->QueryInterface(__uuidof(IUnknown), (void**)&m_pFlash);
	m_pFlash->put_WMode(_bstr_t(_T("Transparent")));
	m_pFlash->DisableLocalSecurity();
	m_pFlash->put_AllowScriptAccess(L"always");
}

void CActiveXUI::InitActive()
{
